    ...
    class OpenIDConnect
      ...
      def access_token
        @access_token ||= lambda {
          ...
          verify_id_token!(_access_token.id_token) if configured_response_type == 'code'

          unless options.allowed_gsuite_domains.empty?
            _id_token = decode_id_token(_access_token.id_token)
            
            unless options.allowed_gsuite_domains.include?(_id_token.raw_attributes['hd'])
              raise CallbackError, error: 'invalid_gsuite_domain', reason: 'Invalid GSuite Domain User'
            end
          end
          _access_token
        }.call()
